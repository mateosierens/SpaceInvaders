...
void Game::startGame()
{
        ...
        try {
                if (!file.good()) {
                        throw std::runtime_error("Couldn't find level file " + level +
                                                 ", please include this in SpaceInvaders directory.");
                }
        ...
        if (currentLevel == 0) {
                ...
                std::shared_ptr<Views::View> shipView =
                    std::make_shared<Views::PlayerShipView>(ship, "./resources/playerShip.png");
        ...
        for (int i = 0; i < j["enemies"].size(); ++i) {
                ...
                std::shared_ptr<Views::View> enemyView =
                    std::make_shared<Views::EnemyShipView>(enemy, "./resources/enemyShip.png");
                enemyView->makeThisObserver(enemy);
                views.push_back(enemyView);
                std::shared_ptr<Controllers::EnemyShipController> enemyController =
                    std::make_shared<Controllers::EnemyShipController>(enemy);
...
void Game::runGame()
{
        ...
        try {
                if (!gameOverMusic.openFromFile("./resources/gameOverSound.ogg")) {
                        throw std::runtime_error(
                            "Couldn't find file with name gameOverMusic.ogg, please include this file"
                            "in the resources folder");
                }
        } catch (std::exception& e) {
                std::cout << e.what() << std::endl;
        ...
        while (window->isOpen()) {
                ...
                Stopwatch::instance().elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                                    Stopwatch::instance().current - Stopwatch::instance().previous)
                                                    .count();
                ...
                while (lag >= MS_PER_UPDATE) {
                        ...
                        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
                                ...
                                if (!player->shotBullet()) {
                                        ...
                                        std::shared_ptr<Entities::Entity> bullet = std::make_shared<Entities::Bullet>(
                                            player->getCoords().first,
                                            player->getCoords().second + player->getEntityHeight() / 2);

                                        // make bullet view and make it observer of the bullet we created
                                        std::shared_ptr<Views::View> bulletView =
                                            std::make_shared<Views::BulletView>(bullet, "./resources/bullet.png");
                                        bulletView->makeThisObserver(bullet);

                                        // make a controller for the bullet we just created
                                        std::shared_ptr<Controllers::Controller> bulletController =
                                            std::make_shared<Controllers::BulletController>(bullet);
                                        ...
                                        // playerShipController now uses update function so it needs to be in list of
                                        // controllers

                                        for (const std::shared_ptr<Controllers::EnemyShipController>& enemy :
                                             enemyShips) {
                        ...
                        for (const std::shared_ptr<Controllers::Controller>& controller : controllers) {
                                // if bullet flies out of the window we want to delete it so we also want to delete the
                                // controller out of the vector of controllers
                        ...
                        for (int j = 0; j < enemyShips.size(); ++j) {
                                if (enemyShips[j]->getEntity() != nullptr &&
                                    enemyShips[j]->getCoords().first < enemyShips[leftmost]->getCoords().first)
                                        leftmost = j;
                                if (enemyShips[j]->getEntity() != nullptr &&
                                    enemyShips[j]->getCoords().first > enemyShips[rightmost]->getCoords().first)
                                        rightmost = j;
                        }
                                    ...
                                    !enemyShips[leftmost]->movingLeft()) {
                                        for (int i = 0; i < enemyShips.size(); ++i) {
                                                if (enemyShips[i]->getEntity() != nullptr && i != leftmost) {
                                                        ...
                                                        newEnemies.push_back(enemyShips[leftmost]);
                                        }
                                }
                                    ...
                                    enemyShips[rightmost]->movingLeft()) {
                                        for (int i = 0; i < enemyShips.size(); ++i) {
                                                if (enemyShips[i]->getEntity() != nullptr && i != rightmost) {
                                                        ...
                                                        newEnemies.push_back(enemyShips[rightmost]);
                                        }
                                }
                                // if direction is the same, use update function for other ships
                                else {
                        ...
                        if (counter == enemyShootPeriod && !enemyShips.empty()) {
                                ...
                                std::shared_ptr<Entities::Entity> bullet = std::make_shared<Entities::Bullet>(
                                    enemyShooter->getCoords().first,
                                    enemyShooter->getCoords().second - enemyShooter->getEntityHeight() / 2);

                                // make bullet view and make it observer of the bullet we created
                                std::shared_ptr<Views::View> bulletView =
                                    std::make_shared<Views::BulletView>(bullet, "./resources/enemyBullet.png");
                                bulletView->makeThisObserver(bullet);

                                // make a controller for the bullet we just created
                                std::shared_ptr<Controllers::BulletController> bulletController =
                                    std::make_shared<Controllers::BulletController>(bullet);
