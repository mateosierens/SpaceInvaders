...
View::View(std::shared_ptr<Entities::Entity>& entity, const std::string& textureFile) : entity(entity)
{
        ...
        try {
                ...
                if (!texture.loadFromFile(textureFile)) {
                        throw std::runtime_error("Couldn't find file " + textureFile +
                                                 ", please include this file in the resources"
        ...
        entity->setEntityWidth(Transformation::instance().toGameCoordsX(spriteWidth) -
                               Transformation::instance().toGameCoordsX(0));
        entity->setEntityHeight(Transformation::instance().toGameCoordsY(0) -
                                Transformation::instance().toGameCoordsY(spriteHeight));

        // convert coords to window coords and set the sprite position
        std::pair<float, float> windowCoords =
            Transformation::instance().transform(entity.get()->getCoords().first, entity.get()->getCoords().second);
...
const sf::Sprite& View::getSprite() const { return sprite; }

void View::setSprite(const sf::Sprite& sprite) { View::sprite = sprite; }

bool View::isDeleted() const { return deleted; }

void View::setDeleted(bool deleted) { View::deleted = deleted; }
} // namespace Views
